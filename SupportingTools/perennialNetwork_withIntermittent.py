...
outpath = r"C:\etal\Shared\Projects\USA\California\SierraNevada\BRAT\wrk_Data\00_CodeTest\Perennial2\WestWalker\NHD_24k_Perennial_Test.shp"
...
def main():
    ...
    arcpy.SelectLayerByAttribute_management('nhd_flowline_lyr', 'NEW_SELECTION', """ "FCODE" = 46003 OR "FCODE" = 33400 """)
    arcpy.SelectLayerByAttribute_management('nhd_flowline_lyr', 'SUBSET_SELECTION', """ "GNIS_NAME" <> '' """)

    # dissolve selected flowlines by name
    flowline_int_dissolve = arcpy.Dissolve_management('nhd_flowline_lyr', 'in_memory/flowline_int_dissolve', 'GNIS_NAME', '', 'SINGLE_PART', 'UNSPLIT_LINES')
    ...
    # add selected flowlines to the perennial stream shp
    arcpy.SelectLayerByLocation_management('nhd_flowline_lyr', 'SHARE_A_LINE_SEGMENT_WITH', 'flowline_int_dissolve_lyr', '', 'NEW_SELECTION')
    with arcpy.da.InsertCursor(flowline_per, ["SHAPE@"]) as iCursor:
        with arcpy.da.SearchCursor('nhd_flowline_lyr', ["SHAPE@"]) as sCursor:
            for row in sCursor:
                iCursor.insertRow([row[0]])
    ...
    arcpy.SelectLayerByAttribute_management('nhd_flowline_lyr', 'SUBSET_SELECTION', """ "GNIS_NAME" <> '' """)
    arcpy.SelectLayerByLocation_management('nhd_flowline_lyr', 'SHARE_A_LINE_SEGMENT_WITH', flowline_per, '', 'REMOVE_FROM_SELECTION')
    int_lines = arcpy.CopyFeatures_management('nhd_flowline_lyr', 'in_memory/int_lines')
    ...
    int_lines_dissolve = arcpy.Dissolve_management('nhd_flowline_lyr', 'in_memory/int_lines_dissolve', 'GNIS_NAME', '', 'SINGLE_PART', 'UNSPLIT_LINES')
    int_gap_lines_dissolve = findGaps(int_lines_dissolve, flowline_per)

    # add itermittent gap to the perennial stream shp
    arcpy.SelectLayerByLocation_management('nhd_flowline_lyr', 'SHARE_A_LINE_SEGMENT_WITH', int_gap_lines_dissolve, '', 'NEW_SELECTION')
    with arcpy.da.InsertCursor(flowline_per, ["SHAPE@"]) as iCursor:
        with arcpy.da.SearchCursor('nhd_flowline_lyr', ["SHAPE@"]) as sCursor:
            for row in sCursor:
                iCursor.insertRow([row[0]])
    ...
    # these are potential network gap lines
    arcpy.SelectLayerByAttribute_management('nhd_flowline_lyr', 'NEW_SELECTION', """ "FCODE" = 46003 OR "FCODE" = 33400 """)
    arcpy.SelectLayerByLocation_management('nhd_flowline_lyr', 'ARE_IDENTICAL_TO', flowline_per, '', 'REMOVE_FROM_SELECTION')
