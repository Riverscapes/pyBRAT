...
dir_list = list.files(pf_path, full.names=FALSE)

dir_list <- dir_list[-1]
dir_list <- dir_list[-1]

for (dir in list){
  validation_file <- paste(pf_path, '/', dir, '/BRAT/', run_folder, '/Outputs/Output_01/02_Analyses/Data_Capture_Validation.shp',sep='')
  huc_name <- strsplit(dir, '_')[0]
  summary_folder <- paste(pf_path, '/', dir, '/BRAT/', run_folder, '/SummaryProducts/SummaryTables', sep='')
  
  print(paste("Running plots for ", dir, sep=""))
  tryCatch(create_validation_plots(validation_file, huc_name, summary_folder),
           error=function(e) {
             e
             print(paste("Oops! --> Error in Loop ",dir,sep = ""))})
  }
...
create_validation_plots <- function(validation_file, huc_name, intercomparison_folder){
  ...
  ifelse(max(e_DamCt==0), return(print('No observed dams')), {
    # pull out NA values
    x = mCC_EX_CT[which(!is.na(e_DamCt))] # estimated dams
    x <- as.numeric(x)
    y = e_DamCt[which(!is.na(e_DamCt))] # surveyed dams
    y <- as.numeric(y)
